...
export default {
  ...
  methods: {
        ...
        .catch(err => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log(
                "Gagal mengambil detail data bongkar basket," +
                  ` kesalahan server (${err.response.status})`
              );
            }
          } else {
            this.app.log(
              "Gagal mengambil detail data bongkar basket, tidak ada jaringan"
            );
          }
        });
    },
        ...
        .catch(err => {
          ...
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log(
                "Detail data bongkar basket gagal diperbaharui," +
                  ` kesalahan server (${err.response.status})`
              );
            }
          } else {
            this.app.log(
              "Detail data bongkar basket gagal diperbaharui, tidak ada jaringan"
            );
          }
        });
    },
        ...
        .catch(err => {
          ...
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log(
                "Gagal mengunduh data bongkar basket dalam bentuk xlsx," +
                  ` kesalahan server (${err.response.status})`
              );
            }
          } else if (err.request) {
    ...
    onDelete() {
      this.app.confirm({
        description:
          "Apakah anda yakin ingin menghapus data bongkar basket ini?",
        promiseCallback: () => {
        ...
        catchCallback: err => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log(
                "Data bongkar basket gagal dihapus," +
                  ` kesalahan server (${err.response.status})`
              );
            }
          } else {
            this.app.log(
              "Data bongkar basket gagal dihapus, tidak ada jaringan"
            );
          }
        }
      });
