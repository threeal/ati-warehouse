  ...
  <v-container>
    <v-row justify="center">
      <v-col cols="12" sm="8" md="6">
        <v-row>
          <v-col cols="12">
            <v-text-field :value="username" label="Nama Pengguna"
                :disabled="fetching" :loading="fetching" readonly
                filled hide-details dense outlined/>
          </v-col>
          <v-col cols="12">
            <v-text-field :value="fullname || 'Anonim'" label="Nama Lengkap"
                :disabled="fetching" :loading="fetching" readonly
                filled hide-details dense outlined/>
          </v-col>
          <v-col cols="12">
            <v-btn v-if="!verified" @click="onVerify()"
                :disabled="fetching" color="primary" block>
              <v-icon left>mdi-account-check</v-icon> Verifikasi Pengguna
            </v-btn>
            <v-btn v-else-if="!admin" @click="onPromoteAdmin()"
                :disabled="fetching" color="primary" block>
              <v-icon left>mdi-account-tie</v-icon> Jadikan Sebagai Admin
            </v-btn>
            <v-btn v-else @click="onDemoteAdmin()"
                :disabled="fetching" color="primary" block>
...
export default {
  ...
  methods: {
        ...
        .catch((err) => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log('Gagal mengambil detail pengguna,'
        ...
        catchCallback: (err) => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log('Pengguna gagal diverifikasi,'
                + ` kesalahan server (${err.response.status})`);
            }
          }
          else {
    ...
    onPromoteAdmin() {
      this.app.confirm({
        description: 'Apakah anda yakin ingin menjadikan pengguna ini sebagai admin?',
        promiseCallback: () => {
        ...
        catchCallback: (err) => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log('Pengguna gagal dijadikan sebagai admin,'
                + ` kesalahan server (${err.response.status})`);
            }
          }
          else {
            this.app.log('Pengguna gagal dijadikan sebagai admin, tidak ada jaringan');
          }
        },
      });
    },
    onDemoteAdmin() {
      this.app.confirm({
        description: 'Apakah anda yakin ingin melepaskan pengguna ini dari admin?',
        promiseCallback: () => {
        ...
        catchCallback: (err) => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log('Pengguna gagal dilepaskan dari admin,'
                + ` kesalahan server (${err.response.status})`);
            }
          }
          else {
            this.app.log('Pengguna gagal dilepaskan dari admin, tidak ada jaringan');
          }
        },
      });
    },
        ...
        catchCallback: (err) => {
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log('Pengguna gagal dihapus,'
                + ` kesalahan server (${err.response.status})`);
            }
          }
          else {
