    ...
    <v-row justify="center">
      <v-col cols="12" sm="8" md="6">
        <v-row>
          ...
          <v-col cols="12">
            <v-text-field
              v-model="cansPerBasketTray"
              label="Kaleng Per Tray Basket"
              type="number"
              :disabled="fetching || submitting"
              :loading="fetching"
              :readonly="!edit"
          ...
          <v-col cols="12">
            <v-text-field
              v-model="cansPerPalletLayer"
              label="Kaleng Per Layer Palet"
              type="number"
              :disabled="fetching || submitting"
              :loading="fetching"
              :readonly="!edit"
  ...
  computed: {
    submitDisabled() {
      return (
        this.submitting ||
        !this.name ||
        !this.cansPerBasketTray ||
        !this.cansPerPalletLayer ||
        !this.cansPerCase
      );
    }
  },
        ...
        .catch(err => {
          ...
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.app.log(
                "Detail jenis produk gagal diperbaharui," +
                  ` kesalahan server (${err.response.status})`
              );
            }
          } else {
            this.app.log(
              "Detail jenis produk gagal diperbaharui, tidak ada jaringan"
            );
          }
        });
    },
      ...
      .catch(err => {
        if (err.response) {
          if (err.response.status === 401) {
            ...
            this.app.log(
              "Gagal mengambil detail jenis produk," +
                ` kesalahan server (${err.response.status})`
            );
          }
        } else {
          this.app.log(
            "Gagal mengambil detail jenis produk, tidak ada jaringan"
          );
        }
      });
  }
