      ...
      Document.find(documentCondition)
        .then(documents => {
          ...
          documents.forEach(document => {
            let productKind = productKinds.find(
              o => o.id === document.productKindId
            );
  ...
  Document.findById(documentId)
    .then(document => {
      if (document) {
        ...
        res
          .status(404)
          .send({ message: `document with id ${documentId} not found` });
      }
    })
    .catch(err => {
      res.status(500).send({ message: err.message });
...
exports.update = (req, res) => {
  ...
  Document.findByIdAndUpdate(documentId, newDocument, {
